...
- `start-all.sh`: First of all, the persistent directory (database) and the backup directory are created if not existent. The profiles is set depending on the input parameter; then the runtime *.env* is prepared — if *override.env* exists it is merged with *env_template.env* via `generate-env.sh`, otherwise *env_template.env* is copied to *.env* only when *.env* does not yet exist (an existing *.env* is kept) — and eventually Docker Compose is started for the file `docker-compose.yml`.
...
- `generate-env.sh`: convenience wrapper that calls `generate_env.py` with the default paths (`docker-compose/env_template.env`, `docker-compose/override.env`, `docker-compose/.env`). Invoked by `start-all.sh` **only when `override.env` exists** (then `.env` is regenerated on every run). Without `override.env`, `start-all.sh` does not run it: it copies `env_template.env` to `.env` only if `.env` is missing, otherwise it keeps the existing `.env`.
